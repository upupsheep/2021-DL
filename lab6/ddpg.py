...
class DDPG:
    ...
    def select_action(self, state, noise=True):
        ...
        with torch.no_grad():
            if noise:
                action = self._actor_net(torch.from_numpy(state).view(
                    1, -1).to(self.device)) + torch.from_numpy(self._action_noise.sample()).view(1, -1).to(self.device)
            else:
                action = self._actor_net(torch.from_numpy(
    ...
    for n_episode, seed in enumerate(seeds):
        ...
        for t in itertools.count(start=1):
            ...
            if done:
                ...
                print('Testing ep {}: , total reward: {}'.format(
                    n_episode, total_reward))
