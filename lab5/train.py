...
def loss_function(predict_distribution, predict_output_length, target, mu, logvar):
    ...
    CEloss = Criterion(
        predict_distribution[:predict_output_length], target[:predict_output_length])
...
def train(vae, loader_train, optimizer, teacher_forcing_ratio, kl_weight, tensor2string):
    ...
    for word_tensor, tense_tensor in loader_train:
        ...
        word_tensor, tense_tensor = word_tensor.to(
            device), tense_tensor.to(device)
        ...
        predict_output, predict_distribution, mean, logvar = vae(
            word_tensor, encoder_hidden_state, encoder_cell_state, c, use_teacher_forcing)
        CEloss, KLloss = loss_function(predict_distribution, predict_output.size(
            0), word_tensor.view(-1), mean, logvar)
        loss = CEloss + kl_weight * KLloss
        total_CEloss += CEloss.item()
        total_KLloss += KLloss.item()
        predict, target = tensor2string(
            predict_output), tensor2string(word_tensor)
...
def evaluate(vae, loader_test, tensor2string):
    ...
    with torch.no_grad():
        for in_word_tensor, in_tense_tensor, tar_word_tensor, tar_tense_tensor in loader_test:
            in_word_tensor, in_tense_tensor = in_word_tensor[0].to(
                device), in_tense_tensor[0].to(device)
            tar_word_tensor, tar_tense_tensor = tar_word_tensor[0].to(
                device), tar_tense_tensor[0].to(device)
            ...
            predict_output = vae.inference(
                in_word_tensor, encoder_hidden_state, encoder_cell_state, tar_c)
            target_word = tensor2string(tar_word_tensor)
            predict_word = tensor2string(predict_output)
            re.append([tensor2string(in_word_tensor),
                       target_word, predict_word])
